...
static void apply_usercmd_delta(const q2proto_clc_move_delta_t *move_delta, const usercmd_t *from, usercmd_t *to)
{
    ...
    if (move_delta->delta_bits & Q2P_CMD_ANGLE0)
        to->angles[0] = q2proto_var_angles_get_short_comp(&move_delta->angles, 0);
    if (move_delta->delta_bits & Q2P_CMD_ANGLE1)
        to->angles[1] = q2proto_var_angles_get_short_comp(&move_delta->angles, 1);
    if (move_delta->delta_bits & Q2P_CMD_ANGLE2)
        to->angles[2] = q2proto_var_angles_get_short_comp(&move_delta->angles, 2);

    // read movement
    if (move_delta->delta_bits & Q2P_CMD_MOVE_FORWARD)
        to->forwardmove = q2proto_var_coords_get_int_unscaled_comp(&move_delta->move, 0);
    if (move_delta->delta_bits & Q2P_CMD_MOVE_SIDE)
        to->sidemove = q2proto_var_coords_get_int_unscaled_comp(&move_delta->move, 1);
    if (move_delta->delta_bits & Q2P_CMD_MOVE_UP)
        to->upmove = q2proto_var_coords_get_int_unscaled_comp(&move_delta->move, 2);
    ...
    if (move_delta->delta_bits & Q2P_CMD_IMPULSE)
        to->impulse = move_delta->impulse;

    // time to run command
    to->msec = move_delta->msec;

    // light level
    to->lightlevel = move_delta->lightlevel;
}
